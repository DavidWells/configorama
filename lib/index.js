      ...
      return Promise.resolve().then(() => {
        return this.populateObjectImpl(this.config).finally(() => {
          ...
          if (ENABLE_FUNCTIONS) {
            ...
            traverse(this.config).forEach(function (rawFunction) {
              if (typeof rawFunction === 'string' && rawFunction.match(/> function /)) {
                console.log('RAW FUNCTION', rawFunction)
                const str = rawFunction.replace(/> function /g, '')
                console.log('str', str)
                const func = cleanVariable(str, varSyntax, true)
                console.log('func', str)
                const funcVal = transform(func)
                const hasObjectRef = rawFunction.match(/\.\S*$/)
...
Make sure the property is being passed in correctly
    ...
    // IS WORKING AFTER POPULATION

    const isOn = true
    // console.log('xxxxxproperty', property)
    if (isOn && property && typeof property === 'string') {
...
Like so: \${${variableString}, "fallbackValue"\}. ${JSON.stringify(this.deep)}`)
          }
    ...
    if (variableString) {
      ...
      if (fallbackValue) {
        // recurse on fallback and check again
        console.log('GOOOOO', fallbackValue)
