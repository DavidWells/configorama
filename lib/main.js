 ...
 Verify the ${varDisplay} in config at "${configValuePath}"`
  ...
  populateVariables(properties) {
    ...
    return map(variables, (valueObject) => {
      ...
      return this.populateValue(valueObject, false, '_populateVariables')
        .then((populated) => {
  ...
  populateVariable(valueObject, matchedString, valueToPopulate) {
    ...
    let foundFilters = []
    if (hasFilters) {
      foundFilters = hasFilters[1].split('|').map(filter => filter.trim()).filter(Boolean)
    }
    ...
    if (property === matchedString) {
      if (DEBUG_TYPE) {
        console.log('DEBUG_TYPE total replacement')
      }
      ...
      // if (!valueToPopulate.match(this.variableSyntax) && foundFilters.length > 0 && !wasDeep) {
      //   property = foundFilters.reduce((acc, filter) => {
      //     return this.filters[filter](acc)
      //   }, property)
      // }
    ...
    // partial replacement, string
      ...
      if (DEBUG_TYPE) {
        console.log('DEBUG_TYPE isString')
      }
      ...
      if (currentMatchedString.match(this.variableSyntax) && !valueToPopulate.match(this.variableSyntax) && valueToPopulate.match(/}$/)) {
    ...
    // partial replacement, number
      ...
      if (DEBUG_TYPE) {
        console.log('DEBUG_TYPE isNumber')
      }
    ...
    //  property = replaceAll(matchedString, String(valueToPopulate[0]), property)
      ...
      if (DEBUG_TYPE) {
        console.log('DEBUG_TYPEisObject')
      }
      ...
      if (
        ...
        matchedString.match(this.variableSyntax) && property.match(this.variableSyntax)
      ) {
        const isVar = /^\${[a-zA-Z0-9_]+:/.test(property)
        if (isVar) {
          // console.log('INSIDE', property, matchedString)
          // console.log('isVar', isVar)
          throw new Error(`Invalid variable syntax "${property}" resolves to "${replaceAll(matchedString, objStr, property)}"`)
        }
      }
...
`
    ...
    /* Apply filters if found */
    //console.log('> property', property)
    if (foundFilters.length > 0 && (typeof valueToPopulate === 'string' && !valueToPopulate.match(deepRefSyntax)) && !property.match(this.variableSyntax)) {
  ...
  getValueFromSource(variableString, valueObject, caller) {
    ...
    if (filters) {
      ...
      // console.log('valueObject', valueObject)
      // console.log('t', t)
      const _filter = string.split('|').filter((value, index, arr) => {
        return index > 0
      }).map((f) => {
        return trim(f)
      })
      ...
      // .fire Need to correctly parse filters
      // @TODO @DWELLS @TODO
      // @TODO @DWELLS @TODO
      // @TODO @DWELLS @TODO
      // @TODO @DWELLS @TODO
      ...
      // console.log('xnewHasFilter', newHasFilter)
      ...
      const valuePromise = resolverFunction(
        ...
        if (val === null || typeof val === 'undefined' ||
          ...
          if (valueCount.length === 1) {
            // console.log('valueObject.path', valueObject.path)
            throw new Error(`Unable to resolve variable ${propertyString} at path ${(valueObject.path) ? `"${arrayToJsonPath(valueObject.path)}"` : 'na'}

Fix this reference or provide a valid fallback value.
        ...
        const newValue = newUse.reduce((a, c) => {
          ...
          if (typeof c.filter === 'function') {
            if (c.args) {
              ...
              return c.filter(theValue, ...c.args, 'from getValueFromSource')
            }
            this.filterCache[pathValue] = (this.filterCache[pathValue] || []).concat(c.filterName)
...
function replaceAll(replaceThis, withThis, inThis) {
  ...
  const pat = new RegExp(replaceThis.replace(REPLACE_PAT,"\\$&"),"g")
  // console.log('replaceAll pat', pat)
