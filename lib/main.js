      ...
      const valuePromise = resolverFunction(
        ...
        if (val === null || typeof val === 'undefined' ||
          ...
          if (valueCount.length === 1) {
            throw new Error(`Unable to resolve variable ${propertyString} at path ${(valueObject.path) ? valueObject.path.join('.') : 'na'}

Fix this reference or provide a valid fallback value.

Set a fallback value like so: \${${variableString}, "fallbackValue"\}
`)
          }
