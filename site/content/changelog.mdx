...
The `inspect` command unifies requirements, audit, and graph as views; the focused verbs remain as compatibility aliases. `capabilities` exposes the contract (commands, formats, error codes) for agents. `eval` and `if` now block constructor/prototype access (`blocked_eval_escape`), so the sandboxed-expression guarantee is enforced, not just documented.
