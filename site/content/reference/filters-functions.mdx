...
Functions include `split`, `join`, `length`, `merge`, `upperKeys`, and `md5`. See [debug resolution](/guides/inspect-config#debug-resolution) for failure behavior and [requirements schema](/reference/requirements-schema) for how filters appear in metadata.
