...
The `inspect`, `requirements`, `audit`, `graph`, and `capabilities` commands emit JSON errors by default; pass `--error-format human` for the boxed form. Resolve mode is the reverse: boxed by default, `--error-format json` to opt in. `configorama capabilities` exposes the full code registry under `errorCodes`.
