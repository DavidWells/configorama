...
If a Markdown file has no frontmatter, the result contains only `_content`. The Markdown body is not part of variable resolution, so `${self:_content}` in frontmatter does not read the body. If frontmatter already defines `_content`, Configorama preserves that explicit value and stores the body separately.

```md filename="page.md"
---
title: ${opt:title, "Docs"}
_content: custom frontmatter value
---
...
Files without extensions are parsed by content sniffing. Configorama recognizes common JSON, YAML, and TOML shapes, including JSON arrays and YAML files with a `---` document marker.

```sh
configorama ./config-json
configorama ./config-yaml
configorama ./config-toml
```

When a file has a recognized extension, the extension wins over content sniffing. Prefer normal extensions for committed config files, and treat extensionless parsing as compatibility for generated files, dotfiles, and platform-specific config inputs.
