...
The setup wizard is for local setup and one-off operator runs. Configorama can inspect a file before resolving it, build a graph of the values it depends on, and use that graph to walk someone through configuring the file.

That graph includes environment variables, CLI options, file references, self references, defaults, type filters, allowed values, sensitivity, and prompt text. The wizard turns those dependencies into questions, then resolves the config with the answers for the current run.
