...
Use the requirements view when you need to know what a config needs before you resolve it. This guide is for CI jobs, agents, and setup flows that need a stable JSON contract for missing environment variables, option flags, defaults, allowed values, sensitivity, and comments from the config file.
...
configorama inspect config.yml                        # requirements alongside graph and audit

# compatibility alias
configorama requirements config.yml
```

`requirements` is the requirements view of the unified `inspect` command. Use the focused view when you only need the input contract, or `inspect` with no `--view` when you also want the dependency graph and audit findings in one call.
...
<Callout type="warning">
  The requirements view does not mean every dynamic target can be known statically. A path such as `${file(./${opt:stage}.yml)}` is reported with inner variables and partial dependency information.
</Callout>
