...
Embedded code paths such as Lambda `Code.ZipFile`, CloudFront `FunctionCode`, shell `UserData`, and dynamic references like `{{resolve:ssm:...}}` should pass through unless a Configorama source is explicitly supported there. See [safe inspection](/guides/safe-inspection) for trust policy, [the resolution model](/concepts/resolution-model) for pass-through behavior, and [API reference](/reference/api) for resolver settings.
