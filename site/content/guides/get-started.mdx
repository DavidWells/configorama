...
Variables use `${...}` syntax. But it is configurable to use other delimiters if you prefer. See [bring your own syntax](/guides/bring-your-own-syntax) for more details.

Variables have the shape `${source, fallbackDefaultValue}`. 

Variables can have multiple fallbacks. For example, `${source, sourceTwo, defaultValue}`.
...
| Use case | Formats |
|---|---|
| Data config | YAML, JSON, JSON5, TOML, INI, HCL |
| Content metadata | Markdown frontmatter |
| Trusted executable config | JS, TS, ESM |

Use data formats for normal config, [eval variables](/variables/eval) for small decisions, and [JS or TS config](/guides/executable-configs) only when you intentionally trust executable code. If `${...}` conflicts with another platform, [bring your own syntax](/guides/bring-your-own-syntax).
...
- [Inspect config](/guides/inspect-config)
- [Setup wizard](/guides/setup-wizard)
- [First config](/guides/first-config)
- [Bring your own syntax](/guides/bring-your-own-syntax)
- [Executable configs](/guides/executable-configs)
- [Eval variables](/variables/eval)
- [Filters and functions](/filters-functions)
- [Variable sources](/variable-sources)
- [Resolution model](/concepts/resolution-model)