...
Dotenv files can be read directly when the dependency should stay explicit and should not mutate `process.env`.
...
Prefer key-level reads for dotenv files. `file(.env)` imports every key in the file and audit reports it as a medium-severity sensitive file read; `file(.env).API_KEY` and `file(.env):API_KEY` are still sensitive, but are reported as key-scope reads.
