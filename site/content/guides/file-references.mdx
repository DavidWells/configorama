...
That keeps the dependency visible without committing the value. If secrets live somewhere else, provide a [custom resolver](/guides/custom-resolvers) instead of a file reference. Custom resolvers can read from local stores such as the macOS Keychain or 1Password CLI, or from remote stores such as AWS Secrets Manager, GCP Secret Manager, Vault, or your own internal service.
...
Missing files can fail, use fallback values, or pass through when unknown refs are explicitly allowed. For exact source syntax, see [variable sources](/reference/variable-sources). For root restrictions and safe-mode behavior, see [safe inspection](/guides/inspect-config#audit-risk).
