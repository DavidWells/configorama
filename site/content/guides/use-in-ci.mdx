...
steps:
  - run: npm ci
  - run: configorama inspect config.yml --view requirements > requirements.json
  - run: configorama inspect config.yml --view audit > audit.json
  - run: configorama inspect config.yml --view graph --format json > graph.json
