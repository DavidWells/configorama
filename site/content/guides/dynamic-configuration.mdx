...
Conditions and expressions are data-flow helpers, not JavaScript execution:

```yaml filename="config.yml"
isProd: ${eval(self.stage == "prod")}
replicas: ${if(self.isProd, 4, 1)}
```
...
YAML anchors and merge keys can still be useful before variable resolution, especially for shared defaults. Use explicit type filters such as `Number`, `Boolean`, and `Json` when dynamic values must preserve non-string types. For deeper mechanics, read [the resolution model](/concepts/resolution-model) and [variable sources](/reference/variable-sources).
