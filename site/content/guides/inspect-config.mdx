...
The audit view is for repositories you do not automatically trust. It reports risk surfaces before resolution, especially executable config, JS/TS file references, custom variable sources, dotenv loading, and file reads.
...
Dotenv file references are reported as sensitive local file reads. `${file(.env)}` is medium severity because it imports the whole dotenv file; `${file(.env).API_KEY}` and `${file(.env):API_KEY}` are low severity key reads with `sensitive: true`, `sensitivityReason: "dotenv_file"`, and `dotenvReadScope: "key"`. Audit reports the reference shape, not the resolved secret value.
...
Safe mode defaults file and text references to the config directory unless you pass allowed roots. Root restrictions block traversal outside configured roots, and safe mode also blocks dotenv mutation, custom variable sources, and custom functions unless policy explicitly permits them.
